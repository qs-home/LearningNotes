...
## final 
> ### final特点
> final可以修饰属性、方法和类，分别表示属性不可变、方法不可覆盖和类不可被继承。

- 1. final属性：被final修饰的变量不可变（引用不可变）。
- 2. final方法：不允许任何子类重写这个方法，但子类仍然可以使用这个方法。
- 3. final参数：用来表示这个参数在这个函数内部不允许被修改。
- 4. final类：此类不能被继承，所有方法都不能被重写。
- 5. final变量，是不可以修改的，是常量。
